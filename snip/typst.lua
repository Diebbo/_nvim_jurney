...
#figure(
  ...
  s({
    trig = 'to', -- exponentiation
    name = 'Exponentiation',
    snippetType = 'autosnippet',
  }, {
    -- places next writting into the exponent ^( ... )
    t '^(',
    d(1, get_visual),
    i(0),
    t ')',
  ...
  s(
    { trig = 'ct', name = 'ct' },
    {
      t '#import "@preview/fletcher:0.5.8" as fletcher: diagram, node, edge',
      t { '', '' },
      t { '', '#let bone = math.bb("1")' },
      t { '', '#let btwo = math.bb("2")' },
      t { '', '#let comp = $compose$' },
    }
  ),
}
